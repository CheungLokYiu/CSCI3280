...
def replace_audio(infilename, start, end, replace_data, old_data):
    ...
    new_audio_data = [replace_data[i] if (int(start*bytesperframe*framerate)-int(start*bytesperframe*framerate)%2) <= i < int(end*bytesperframe*framerate)-int(end*bytesperframe*framerate)%2 else old_data[i] for i in range(len(old_data))]
    # new_audio_data = audio_data[(int(start*bytesperframe*framerate)-int(start*bytesperframe*framerate)%2):int(end*bytesperframe*framerate)]
    return new_audio_data
...
convert_audio_to_wav(frame1, output_file) # raw audio file save to wav file  
play_wavaudio(output_file)
data = open_and_edit(output_file+'.wav', 0, 10, 1)

streamplay(data)
savefile('output_edit1.wav', data)
play_wavaudio('output_edit1')
frame2 = start_record(3)
replace_audio('output_edit1', 3, 6, get_record_data(frame2), data)
convert_audio_to_wav(data, 'replace.wav')
